...
Focus was on the  Middle layer Super Output Areas - 2011" (MSOA) which gives around 7000 areas to work with.MSOA areas, but I tested on a subset of the SOA at the end.
...
And then created a function to download all the relevant census tables at once, for a specific geography type (SOA, MSOA etc.).
...
Starting with the Sex data set, which also includes the total number of people.  It can be seen that the total number of residents listed in the Occupation table is roughly half of the total in this table.  On the other hand, the Sex data is fairly static, with more women then men everywhere.  I have used the female ratio, although this is for the full population rather than the Occupation group, which may not be right.
...
The following were retained, combining the Ireland values with the rest of the EU, as there were not so many people:
...
The code is intended to support different area sizes, and therefore repeatable functions were created, as shown earlier, taking in various geography tables as input.
...
Next the functions were then applied to the middle layer super output areas (MSOA), which produced a data set of 7201 observations with 62 variables, of which two are the name and the code for the area, and one is the $y$, the ratio of people in managerial and professional occupations.  This file when saves as an R object is 3MB.
...
Here is the mean, maximum and minimum, which shows a decent spread.
...
This will only work if the algorithms treat the number representing the geographic area as a category variable - if it treats it as a continuous variable than it will not work, as the numbers are unrelated to underlying distributions.  A better way to do this would be to create a feature per region, and then give a 1 or 0 as to whether the MSOA/SOA is in that region.

This would be unwieldy with hundreds of areas as columns, and so we need to summarise.  There is a lookup from the local area (LAD) to a smaller number of regions in England (Wales is effectively a single region), as described here.
https://geoportal.statistics.gov.uk/datasets/local-authority-district-to-region-december-2018-lookup-in-england

This lookup is then used to can use this to create a further lookup to a specific region, which is in turn used to create columns.  I will use names for ease of understanding, and add to the geo_lookup created earlier.
...
In addition, I included some code to be able to handle the different geo types, the SOA and MSOA areas, as they require slightly different lookups for the geographic region.
...
and applying to the test data gives a value of 0.09647.  I tested this, with a sequence around the $\hat{\mu}$ value, and this is the lowest RMSE for a single estimated figure.
...
Some algorithms do not handle categorical features well.  Although there is potentially usefule information, as in, there is an association between the categories and the outcome, it is not possibly to calculate a correlation where there are few values.  Therefore some algorithms will struggle, and therefore I will remove them initially, and then test and bring them in later.
...
I created with this input a new "train_small" with 5832 observations of 36 variables, 35 features and the outcome y.
...
I have not found a definitive list of families, at least in part as there are many overlapping approaches, and it is a matter of opinion as to where algorithms fit.
...
Here I plot the 9 most important features for GLM vs the outcome y, and visually you can see the link.
...
```{r knn with normalised data, eval=FALSE}
...
A visual inspection of the y_hat for glm and lasso models, they are the same, however they are not identical using the "identical" function, but with "all.equal" which allows for some slight differences they are identical across the RMSE and $\hat{y}$ predictions.
...
I tried this model as part of the tree family, a more complex one than CART.  It performed well, but slowly with the smaller data set, though.
...
This is slow, no feature importance, but providing good results.
...
For a final model I tried another variant of the SVM model is the best performing so far after the GLM, and a much discussed model type.  There are a number of variants which use different "kernels" which create the abstracted relationships between the features. I chose the SVM Radial, which although using the same package, "kernlab", it has a different kernel, which means it handles the non-linear elements differently.
...
Similarly for the SVM algorithm, focusing on the top 20 to 40 features, incrementing by 2 each time.  This seems to have a drop around the 24 and then flattish until dropping again around 32/33.
...
This indicates that the algorithms should use at least the top 24 features, as there is a bit drop just beforehand, and potentiall around 28 to have the biggest impact on the overall RMSE.
...
I will use the final model developed with the train/test data to train the final predictions.
...
Perhaps it is due to the size of the data set again, so I will try with subsets as for the SVM Radial.  GAM top 28 with 5000 observations is not so good, but with 10,000 it is better than with the full set of observations, at 0.045806 close to the GLM model with 0.045790.  However, with 15,000 observations the performance was worse that the 10,000 sample.
...
```{r graph of qualification features vs y soa, message=FALSE}
...
```{r graph of qualification features vs y msoa, message=FALSE}
...
```{r stratification by white uk proportion, message=FALSE}
...
```{r graph of industry features vs y soa, message=FALSE}
...
```{r graph of industry features vs y msoa, message=FALSE}
...
```{r graph of household features vs y for soa, message=FALSE}
...
```{r graph of household features vs y for msoa, message=FALSE}
...
```{r graph of marital status vs y soa, message=FALSE}
...
```{r graph of marital status vs y msoa, message=FALSE}
...
Of the features, loneparent is important, as is construction, both of which had strongly negative correlations.  Ethnicities do not gain in importance, with only white irish which was in the top list earlier.  Age and female ration are in the top 10 which seem odd, I would not expect them to be strongly influencing the outcome.
...
```{r graph of ethnicity vs y for soa, message=FALSE}
...
```{r graph of ethnicity vs y for msoa, message=FALSE}
...
```{r plots of ethnicity vs level4 qualifications for soa, message=FALSE}
...
```{r y vs level4 stratified by black african, message=FALSE}
...
```{r y vs level4 stratified by black other, message=FALSE}
...
```{r graph showing proportions per ethnicity, message=FALSE}
# graph
...
There were too many features, and some could probably be removed with little impact, but I did not test that specifically.
...
In general interpretability from the models was not very clear.  Many have no indication of feature importance, or ways to get information out, without running many times with different sets of features.  In addition, the investigation at the end appeared to show som implications from the ethnicity features which were not highlighted in model, where things like lone parent came out as more important.  I suspect this is because there were a lot of features, which are correlated but it does make it hard to use machine learning models to make conclusions other than predictions.

The "high correlation" function in caret did not work well at helping feature selection, as some of the listed highly correlated features were the key ones.  Feature importance seemed much more effective in use.  In future I would choose a simple algorithm and see the feature importance as a way of identifying the ones to prioritise.
