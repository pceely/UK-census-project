...
As for the marital features.  There are a number of religions with a small number of people, and it may make sense to aggregate them in to "other".  Hoever, with the same cutoff of 0.5% as earlier, only one religion is summarised, "Jewish", therefore it can be left as it is.
...
|gam      | regression         | good on smaller| 0.036524| slow   | yes |
...
The smaller set was also used with th KNN model, but although it was the best it had produced by some distance with an RMSE of 0.021077, it is still some distance behind the leading models.
...
Creating an ensemble to include the GAM and GAM Loess just created, with five models (GLM, SVM Linear, SVM Radial, GAM, GAM Loess), averaging the prediction, the result is an RMSE of 0.0149383, vs the SVM Radial model of 0.014525, and only slightly better than the last ensemble of three (GLM, SVM Linear, SVM Radial), which is maybe because the strong performance of the GAM is counterbalanced by the relatively poor GAM Loess.
...
However, looking at the data, this appears to support the general themes of the discussions.
...
The optimal result for the MSOA data was from an ensemble of two models Support Vector Machines with a radial kernel (svmRadial) and Generalized Additive Model using Splines (GAM), where the GAM model needed a reduction of numbers of features to 28 in order to complete the calculations.
...
In this case again an ensemble of 2 models was best performing, Support Vector Machines with a radial kernel (svmRadial) again, but Stochastic Gradient Boosting (GBM) as the second.
...
* "lone parent" and "divorced" have clear negative impacts, and this should be investigated further, but this implies poor roles for the residents in these households.  Possibly more support should be provided, probably around child care, and this probably impacts women more than men.
