...
The most granular data is provided for "output area" (OA) an area defined for the census, of at least 100 people, but aiming for larger than 125 households, which provides over 130,000 data points providing a good level of detail and variability.  This is a very large data set, and will be difficult to handle, for someone new to this, and therefore I will focus this study on a smaller data set, however, I will test with the SOA at the end to validate the work.

For this project, I will be using the Middle layer Super Output Areas - 2011" (MSOA) which gives around 7000 areas to work with.
...
In summary, for this study I will be using the MSOA geographic areas for England and Wales, with data sourced from 9 tables from the nomis census data.
...
```{r priority ethinicities, eval=FALSE}
...
#### Sex/gender in the working age population

Earlier I created a feature for sex/gender, but that was for the full population, and I think it would be better to create one for working age, which I will do here.Here I am intending to create a new female ratio, using the same 25 to 64 group as for ethnicities, and also using the same source data.
...
Following is the code to carry out the mapping, which groups the over 7000 MSOAs and 130000 SOAs in to 348 local areas.
...
I have carried out the analysis so far with MSOA data, around 7000 observations, nationwide.  I will now work with the SOA data, considerably more detailed.

Rather than all 130,000 entries, I will focus on two SOA areas, London and Yorkshire and The Humber, as these will have some variety in the types of areas, roles and people.  The London set had 25,053 observations, and the Yorkshire had 17,275, making 42,328 in total.
...
```{r import london and yorkshire data}
...
Then I combined in to a single data set (adding rows together) before creating the main/validation sets, and the train/test.

```{r create soa main and train sets}
...
Firstly I created the initial baseline RMSE based on the mean, which is 0.133419, much worse than the MSOA data which was 0.096470.  I expected this, as I was hoping that the variability with the smaller areas would be greater.  It could be that focusing on two more extreme areas in the UK could exacerbate this.

```{rmse baseline for SOA}
#### baseline rmse and rmse function
# load the test and train data
load("rda/test_soa_final.rda")
load("rda/train_soa_final.rda")
...
And the table comparing demonstrates this, with the same mean, but larger standard deviation and broader max and minimum.
...
#### Developing models with the train & test sets

Initially I will model with the top three models with feature importance from the MSOA work, to be able to develop the top 28 features for the GAM model:

* GLM
* GBM
* GLMBoost. 

Then I will model the top 3 models:

* GLM
* SVM Radial
* GAM top 28 features

And build an ensemble with the SVM Radial and GAM 28.
...
GBM took around fifteen minutes, and achieved a similar result to GLM, slightly better, but much worse than the result with the MSOA data.  For importance, it was almost completely reliant on the level4 qualification.
...
And GLM Boost was very quick, and the worst of the three models.
...
With these three models I was able to build the top 28 features, and the data set with the top 28 features.

```{r top 28 features with SOA}
...
I therefore tried with the top28 training set, but this failed after taking much longer than the earlier training.
...
This probably means that the algorithm struggles with the amount of data, so I will try again with a smaller data set, to see if we can get it to work at all, it took a long time even with 5 features.  I followed the same process as before, creating a "small" data set with 5000 observations, and even with only 5 features, the result was pretty good, much better than with the full set earlier, with 0.0488267.

```{r svm radial SOA with 5 features}
...
I have increased the number of features to improve the SVM Radial predictions, with 28 features and 5000 observations this provides good results, 0.0452578, which is now second to the GBM model, with only a seventh of the observations available.

```{r svm radial increasing model sizes}
...
Perhaps it is due to the size of the data set again, so I will try with subsets as for the SVM Radial.  GAM top 28 with 5000 observations is not so good, but with 10,000 it is better than with the full set of observations, at 0.045806 close to the GLM model with 0.045790.  However, with 15,000 observations the performance was worse thatn the 10,000 sample.
...
Previously for the MSOA model, I built an ensemble with the SVM Radial and GAM with 28 features.
...
I put together the ensemble of two, which provided the best result of an RMSE of 0.0431886.

```{ensemble for the SOA data}
...
Looking at the results, it is not too illuminating.  In general the distribution is a bit wider than the MSOA areas, nothing too obvious.  Apart from the higher predictions, as the models pretty much predict no proportions over 0.6, whereas the real data has values as high as 0.7.  This could be the cause of the difference with the earlier results, as the larger MSOA areas would tend to average out and reduce the higher values.
...
Returning back to the motivation for this topic to consider achievements of top jobs in the context of the Black lives matter society wide debate.  
https://www.theguardian.com/business/2020/jul/28/bame-representation-uk-top-jobs-colour-of-power-survey 

In this section I will look at the feature importance and correlation and see if there is data to enlighten the discussion, specifically focusing on the ethnicity features. 

## Feature importance 

I carried out an investigation earlier in to the most important features for the MSOA areas with the following features are positively correlated:
...
```{r graphs for top features for SOA}
...
```{r  top feature correlation for SOA}
#looking at correlation with y
data.frame(cor(train_set_final)) %>%
  select(y) %>%
  rownames_to_column(var="feature") %>%
...
Looking at all the qualification features, and mapping against the y values.  Here only the Level4 and no qualification have values about 0.25, which is why they appear in the models.  In addition, all are negatively correlated apart from Level 4, with level 1 and no qualification clearly negative.

```{r graph of qualification features vs y soa}
...
```{r graph of qualification features vs y msoa}
...
Looking more closely at the level 4 qualifications for MSOA and SOA areas with the proportion of white UK people.  In both cases it looks pretty linear, which is the main driver of the linear model strengths.  Visually, it looks like the areas with a high white proportion tend to be closely correlated and the others areas less so.

```{r plot of level4 against y}
...
This can be illustrated further with stratification of the level4 vs y by white uk, where you can see that for lower proportions of white UK, the link with education is less important.  I have only used SOA data here as there is more detail and variation.

```{r stratification by white uk proportion}
...
And here is the correlation values, which also illustrate that below 60% the link between qualification and professional and managerial occupations is weaker.

```{correlation between qualification and y stratified}
...
Looking at the industry predictors in the same way to the qualifications.  Here most of the features have flatish results, apart from Industry KLMN, positive, and Manufacturing, construction and GI, negative.  Also, OPQ is the only industry along with KLMN which has high proportions in areas with high proportion of senior roles, even though overall it is flat.
...
```{r graph of industry features vs y soa}
...
```{r graph of industry features vs y msoa}
...
In the household features, loneparent has a clear negative correlation, although it does not come out as an important feature, perhaps it is closely related to another feature.  Married family appears on the list, but has an only slight positive gradient.

```{graph of household features vs y for soa}
...
For Marital predictors, most of the predictors have quite low values, apart from married and single which have a large spread, with married positively correlated.  Divorced is also clearly negative.

```{r graph of marital status vs y soa}
...
```{r graph of marital status vs y msoa}
...
I am interested in feature importance, and as qualifications are so importance, I will try without the qualification to see what remains.  I will focus on the GLM model as it was high performing and fast, with importance.
...
```{r prediction without qualification data}
## msoa data
...
Of the features, Loneparent is important, as is construction and age.  Ethnicities do not see so important, other than white irish.


## Ethnicity deep dive

Part of the original motivation for this project was the reported lack of opportunities for certain ethnic groups.

Interestingly, in the earlier analysis, led by the models and the raw data, ethnicities do not generally come out as significant factors, apart from positive correlation with white Irish and white other.

In this section I will look specifically at ethnic features and see if there is anything there, similarly to the reports.

Looking at the plots between the ethnicities and Y, proportion of managerial and professional roles, it can be seen that the fit lines are fairly flat apart from white other (positive), but there are lots of graphs which show a triangle shape with low proportions of senior roles in areas with high concentrations of that ethnicity, particularly Black african, Black other, Bangladeshi and Pakistani.  These are the communities often highlighted in the press as having poor outcomes, backed up by the data here.

```{graph of ethnicity vs y for soa}
...
```{graph of ethnicity vs y for msoa}
...
```{r correlation and p value for ethnicity and y soa}
...
```{r correlation and p value for ethnicity and y msoa}
...
Next, carrying out a similar analysis against the level4 qualifications.  Here, as expected the results are similar to the more senior managerial and professional occupations, with white other, white Irish and Chinese positive, and white UK and Pakistani negative.

```{r plots of ethnicity vs level4 qualifications for soa}
...
```{r table of correlation qualification and ethnicity in soa}
# extract the ethnicity features for SOA
...
```{r table of correlation qualification and ethnicity in msoa}
# extract the ethnicity features for MSOA
# extract the p-value result for level4
train_msoa_level4_p <- 
  data.frame(train_msoa_rcorr$P) %>%
...
I can stratify the data to see how the relationship between qualifications and senior occupations varies with the proportion of black african and black other residents.  I limited to stratas with at least 50 samples, and you can see that the relation between occupation and qualification weakens with more Black African residents.

```{r y vs level4 stratified by black african}
...
```{r y vs level4 stratified by black other}
...
Lastly I will look at the overall outcomes in England and Wales (the detail is not available for the UK) for senior managerial and professional occupations and level 4 qualifications, with their ratio by ethnicities.
...
Unfortunately one of them only has summarised data, which I will use.  This has a single figure for Asian, so masking the differences between Indian/Chinese, and Pakistani/Bangladeshi achievements.
...
As mentioned, the Asian group includes high performing Chinese and Indian groups, as well as poorly achieving Pakistani and Bangladeshi, and I suspect that the two groups would diverge in terms of both qualifications and occupations.
...
```{r graph showing proportions per ethnicity}
# graph
...
The results for the SOA areas London and Yorkhire and Humber were not as good.
...
note the poor achievements for white uk, both in occupations and qualifications.  and that the best is for white other and Irish, which will be primarily EU migrants.  Think about this as drivers for populism and brexit, the underlying causes.
...
This study was to focus on machine learning, but it would be interesting to extend the work to look at the correlation between the different factors.  For example, "White other" is positively correlated and "Pakistini" and "Bangladeshi" are negatively, and this could be due to education and qualifications (the press mentions the poor school outcomes for some ethinicities), and it would be interesting to investigate which are convoluted and which are causal in terms of drivers.
