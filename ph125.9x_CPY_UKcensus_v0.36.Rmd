...
This project is the final part of the **EDX course HarvardX: PH125.9x Data Science: Capstone**, to apply machine learning techniques to publicly available data sets.  It is inspired by the UCI machine learning repository, to predict wages based on census data, but focusing on the UK, and using the UK 2011 census data. 

During the project a new continuous data set was developed from the 2011 UK census data, which was then cleaned and prepared for machine learning to answer the following challenge:

* predict the proportion of working age people in each area that are one of the managerial and professional occupations

The modelling was carried out by applying many different algorithms to focus down on the most effective ones, and with these building an ensemble.
...
The final data set created from the census had 68 features of the following groups: Age; Ethnicity; Sex/gender; Qualifications; Marital status; Religion; Household composition; Industry; Country of birth; geographic region within England and Wales.  The categories were either summarised or directly taken from the census categories, and the input data was the census tables and the related tables on geographic areas.

There are various levels of detail for the census data, and the main focus was to use the "Middle layer Super Output Areas - 2011" (MSOA) which gives around 7,000 areas/observations.  The models and methodology were tested on a subset of the most granular data "Super Output Areas - 2011", with over 42,000 observations.
...
## Results
<!-- results recap -->
Overall the proportion of people with senior roles is quite predictable given the features provided.  The root mean error of 0.013991 is small given the range of 0.06 to 0.66, and even the less well performing SOA areas achieved 0.043189 with a larger range of 0 to 0.89.

The optimal result for the MSOA data was from an ensemble of two models Support Vector Machines with a radial kernel (svmRadial) and Generalized Additive Model using Splines (GAM).

The optimal result for SOA was also an ensemble of 2 models, Support Vector Machines with a radial kernel (svmRadial) again, but in this case Stochastic Gradient Boosting (GBM) as the second.

The models show that the achievement of senior management and professional occupations is closely linked to the level4 qualification, hence linear regression based algorithms perform well.


## Conclusions
<!-- Conclusion recap -->
The machine learning analysis was able to make a significant stepwise improvement during the project by working with many algorithms and constructing a final ensemble, from a baseline RMSE of 0.091347 to 0.014253 for MSOA.  The best models were based on linear regression and more recent, popular boosting or additive variants.  There were problems working with the larger data sets, however it was possible to generate results.

A key conclusion, reflected in discussion of AI and ML recently is that in general the models lack of interpretability.  Many have no indication of feature importance, and the feature investigation at the end appeared to show some implications which were not highlighted in model outputs. 

By far and away the most important feature for a high quality occupation is achieving good qualifications. In addition age, female ratio, lone parent, and white ethnicities were highlighted.

The analysis of the features does appear to show evidence supporting the issues being raised in the campaigns, press and politics.  For example "white uk" was negatively correlated with both senior occupations and qualifications; in contrast, other white ethnicities, "white Irish" and "white other" were both highly qualified and with the best jobs; there was a relatively poor occupation achievement for "black other" and "black African", despite better qualifications than average.  This data appears to support the themes reported such as a lack of opportunity for non-white ethnicities; and there are 'left behind' poorly qualified white UK areas, contrasting with better qualified non-UK white people over-represented with the best jobs, the majority being born in the EU.  In addition "lone parent" and "divorced" have clear negative correlations for the best occupations. 

The data set worked as a continuous machine learning problem, it behaved well, with sensible results and a variation in different results from different algorithms, although too many features.  With the SOA data there is potential for a very large data set of 130,000 observations in England and Wales, with 68 features, over 6 million pieces of publicly available data. The analysis indicated that all features had benefit for predictions, but the top 28 were most important.


## Document structure
<!-- apology/explanation for the length of the document -->
This report covers all of the analysis and development of the initial data set, the various models, and finally some analysis and visualisation of the features.

The document is long, as it includes a lot of graphs/plots, and the majority of the code.  The code is in turn long due to the large amount of data wrangling to put the data set together from the ran census information, and additionally due to trying modelling on a large number of algorithms to identify the best for the final ensemble.
...
This project focuses on the UK, and uses the UK 2011 census data.  This data does not have individual information, instead providing the numbers of residents within a geographical area, and in addition,  and instead of pay, it uses the occupation types.  Therefore the challenge is to:
...
As this is a continuous problem, it would be appropriate to use a loss function, and this project uses the simplest, the root mean squared error (RMSE) for each geo location area.
...
2.  Import data, create the values to predict and the features, summarising where appropriate
3.  Create project data set and the validation set,  clean up the features
...
7.  Training and testing a variety of algorithms
8.  Building an ensemble, including further feature prioritisation
9.  Finalising a model and running on the validation set
10. Attempt with the more detailed SOA data set to confirm the models
...
<!-- a methods/analysis section that explains the process and techniques used, including data cleaning, data exploration and visualization, insights gained, and your modelling approaches (you must use at least two different models or algorithms); -->
This section describes the data sources, the process to download and develop the features from the raw tables, the creation of the main/validation and train/test data sets, as well as the initial data exploration, and finally, creating the data sets to use for modelling.

The data was downloaded and combined in to 62 columns of data, and from these, the geographic regions were included to create a data set of 68 features and one outcome, being the proportion of managerial and professional roles.

There were some features with little variability and some highly correlated, which may be useful for simplifying the data sets during the analysis.
...
For the initial data processing and  preparation, clean up and cross checking the numbers between the tables the "regions" geographic areas will be used to ease troubleshooting.

The data will focus on England and Wales because there is more detailed data available which enables further clean up (for example, to baseline on the same working age resident numbers and so on).
...
There are other potential predictors such as 'Housing type', but this is possibly related to how much money earned, so effect rather than cause?

It may be possible to map the occupation to earnings with other data, but this was not carried out, such as:
...
This section describes how the data is downloaded and converted in to the features used in the modelling, and includes most of the code used for "data wrangling".

To recap, this study will use the MSOA geographic areas for England and Wales, with data sourced from 9 tables from the Nomis census data published online.

Working with the occupation data initially, the target "y" values are created as a proportion of the residents in an area, so a fraction of 1.  The other data sources will be addressed in a similar way to produce the features, developing values that are proportions of residents, trying to use a similar cohort as the occupation data.


### Data import

A function was created to download the files from Nomis, for a given geography type and table name, saving the csv files to the ~/data directory.
...
There are 9 Occupation types, and we are combining the first two and the remaining seven:
...
The $y$ value is proporition of residents with managerial or professional occupations, simply the sum of residents who have Occupations 1 and 2, divided by the sum of all residents.
...
Typically in the census, there is a breakdown of ages 16-74, as well as 25-64, but those age ranges are still larger than the Occupation total, and excerpt of the table is included.
...
Table: Comparison of resident numbers
...
Table: Comparison of resident numbers and employment
...
There are a number of religions with a small number of people, and it may make sense to aggregate them in to "other".  However, with the same cut-off of 0.5% as earlier, only one religion is summarised, "Jewish", therefore it can be left as it is.
...
## Create main and validation MSOA data set 
...
## Data exploration and visualisation of the MSOA data
...
Following are the list of highly correlated features, which can be seen in a version of the image with the features re-orded, with a number very similar to each other at the left and bottom rows.
...
This section covers working with the machine learning algorithms to develop the optimal model, and exploring the benefits of an ensemble model.  In addition, it covers selection of optimal features for the algorithms, selection of models, and some visualisation of errors.
...
The low variance candidates have a mean <=0.05, standard deviation <=0.015, and maximum <0.15, which are the following.
...
This generates a short list of features to remove.  A cut-off of 0.8 was selected, with illustrations below.
...
The data set for training has 5832 observations of 68 variables.  Following are features that can be removed with limited impact, or to avoid categorical issues, in addition to removing the "geo-code" column as it is neither a predictor nor outcome:
...
It takes no time, and gives a sensible order of importance for the variables, with "no qualification" first.  And it provides a significant improvement in the RMSE from 0.096470 to 0.039643.
...
Following is the plot of the 9 most important features for GLM against the outcome y, to visualise the links between the features and the outcome, y, the proportion of residents with senior manager or professional occupation.  'Level4 qualification', 'no qualification' and 'industry klmn' have the clearest links.
...
It took a few seconds with the smallest training set with a result a little better than the GLM model, 0.038137.  There is no ranking of feature importance.  The model used 9 neighbours.

The larger training sets worked within a few minutes, however, the result was much worse at 0.070994 for the small and 0.070648 for the full set without the categorical variables.
...
This was run on the full data set, and did not perform well at all, worse than predicting the average at 0.102314.  Perhaps with normalising, the algorithm is unable to discriminate features with little variation and link to the outcome from those with big changes for different geographic areas.
...
|"knn" - train_set_knn_final_nocat       | 0.023753|
...
The model runs slowly, a few seconds with even the small set, but a decent RMSE result of 0.039519, but does not provide an importance view.  The model worked with all data sets, is able to handle categorical features, with the best results with the full data set.
...
There is information on importance of variables, and it can handle the categorical features, with a small improvement.
...
Another potentially useful model, especially as there is information on important features.
...
|"bayesglm" - train_smaller    | 0.039691|
|"bayesglm" - train_small      | 0.024448|
...
Table: GAM loess model RMSE results for MSOA, comparing with other smaller data set results
...
Now exploring the tree family further after the poorly performing simple tree model CART-rpart.  The Ranger model is one of the random forest variants, and it was a big improvement over rpart, but slow, due to the many iterations to build the forest of trees.
...
This runs fine on the smaller set, with importance, and is driven by the lone parent as for other models.  It runs quickly, without errors on all the data sets, including the categorical features, and with the final set it is marginally better, but the performance is average.
...
Reviewing the model, it looks like the number of iterations of "boosting" could be increased which would increase the accuracy.  However, it is far away from the leading models, and this would not close that large a gap, and also, that is likely to over fit the specific train/test data sets.
...
Visual inspection shows a small difference with the RMSE and also the y_hats seem very close, tracking each other, with a "Mean relative difference" through the all.equal function of 0.000334.  The project will focus on the svmlinear and not explore this version any further.
...
|ranger    | tree / random forest | ok/good     | 0.018276| v slow | no |
|avNNet    | deep learning      | ok/good      | 0.019568| slow  | no |
|glmboost  | boosted linear     | ok           | 0.020032| fast   | yes |
|knn       | clustering         | poor           | 0.023753| medium | no |
|gam       | regression          | good on smaller| 0.036524| v slow | yes |
|gamLoess  | regression         | good on smaller| 0.038299| v slow | yes |
|pcr       | dimension reduction | poor          | 0.046037| fast   | no |
|rpart     | tree               | poor           | 0.050706| fast   | yes |

mlp - conflict with other packages   
...
An ensemble of the leading algorithms was created as the final model.

In addition to developing the final ensemble model this section attempts to make use of the GAM and GAM Loess algorithms through feature selection, for use in the final ensemble.
...
```{r top 15 and 20 features with GLM, eval=FALSE}
...
```{r glm results with increasing numbers of features}
load("rda/rmse_results.rda")
rmse_results %>% 
...
For the GAM model, this achieves an RMSE of 0.016136, better than all but the SVM Radial model.

Trying with the top 28 features, and this was better still, achieving an RMSE of 0.015935.
...
The smaller set was also used with the KNN model, but although it was the best it had produced by some distance with an RMSE of 0.021077, it is still some distance behind the leading models.
...
Creating an ensemble to include the GAM and GAM Loess just created, with five models (GLM, SVM Linear, SVM Radial, GAM, GAM Loess), averaging the prediction, the result is an RMSE of 0.0149383, compared against the SVM Radial model of 0.014525, and only slightly better than the last ensemble of three (GLM, SVM Linear, SVM Radial), which is maybe because the strong performance of the GAM is counterbalanced by the relatively poor GAM Loess.
...
Creating an ensemble with the best three models, (GLM, SVM Radial, GAM), the result is an RMSE of 0.014395, compared to the SVM Radial model of 0.0145255, so improving and now the best result overall.
...
Next, load the top28 features developed using the train/test data on GLM, GBM and GLM boost, and then run on the GAM model.  Note, the top28 ranking was developed earlier with information from the test as well as train data, but has not been developed with any data from the validation set.
...
The analysis so far has been carried out using the MSOA data, around 7000 observations, nationwide.  Now the same methodology will be used with the SOA data, considerably more detailed and larger, to see how it behaves.
...
### Importing and creating the data set
...
```{r boxplot of SOA features, fig.height=4}
#looking at the features
...
# creating new small and smaller training sets for SOA
...
Now working with the GAM model to see how that will perform.  GAM on the top 28 features with all 34,282 observations produced a result of 0.046186, behind the GLM and GBM models, surprising, and taking a long time, over an hour.  
...
Training with subsets of the data once more, top 28 with 5000 observations is not so good, also taking quite a long time at 25 minutes.  But with 10,000 it is better than with the full set of observations, at 0.045806 close to the GLM model with 0.045790.  However, with 15,000 observations the performance was worse that the 10,000 sample.
...
Feature importance and correlation are analysed to see if there is data to enlighten the discussion, specifically focusing on the ethnicity features.  This investigation does appear to show evidence supporting the issues being raised in the campaigns, press and politics - poorer outcomes for non-white ethnicities, better qualified non-UK white ethnicities over-represented with the best jobs, and perhaps 'left behind' poorly qualified white areas.
...
Given that features from these sources appear in the most important lists it is worth looking in more detail at the following:
...
This can be illustrated further with stratification by 'white uk' of level4 plotted against y, where for lower proportions of 'white uk', the link with qualifications is less important.  Only SOA data is used here to provide more detail and variation.
...
In the household features, 'loneparent' has a clear negative correlation, although it does not come out as an important feature, perhaps it is closely related to another feature, or that it, along with cohabiting has a lowish proportion, less than 0.4.  Married family, one person and cohabiting appear on the important lists, but have only a slight gradient.
...
x   ## Modelling without qualification features
...
Starting with the MSOA data, removing the qualification features, the RMSE is 0.0314370 for GLM, compared to 0.0166809 with the full data, significantly worse, and not so far from the baseline of the mean of 0.0965.

Of the features, 'loneparent' is important, as is construction, both of which had strongly negative correlations.  Ethnicities are not highlighted further, with only 'white irish' which was in the top list earlier.  However, 'Age' and 'female ratio' are now in the top 10.
...
However, looking at the feature data, there are trends which appear to support the general themes of the discussions.

In the visualisation of the distributions of proportions of ethnicities and y, proportion of managerial and professional roles, the fit lines are fairly flat apart from 'white other' and 'white irish' (positive).  But there are lots of graphs which show a triangle shape with low proportions of senior roles in areas with high concentrations of that ethnicity, particularly 'Black African', 'black other' and 'Pakistani'.  These are the communities often highlighted in the press as having poor outcomes, backed up by the data here.

It is also interesting to compare the outcomes for 'white uk', 'white other' and 'white irish', where the latter two will primarily be migrants from EU, and they have very clear positive correlation, with white UK moderately negative - this indicates that EU migrants have better roles than white UK residents, which may be the underlying driver for Brexit.
...
Carrying out a similar analysis against the level4 qualifications, as expected the results are similar to the occupations, with white other, white Irish and Chinese positive, and white UK and Pakistani negative.
...
However, there are differences, with 'Black African' and 'Black Other' appearing to have worse occupation achievements than their educational achievements.  This is more clear looking at the values, with correlation between areas with more 'Black African' residents and Level4 qualification as -0.089220, compared with -0.197096 for the senior occupations (SOA); for 'Black other' residents, the level4 qualification has a correlation of -0.013246, whereas the occupation has a correlation of -0.109491 for SOA, with similar results in the MSOA, with a roughly -0.1 between the qualification and occupation.
...
ethnicity_y_25_64_raw <- data %>%
  ...
  select(contains("geography") | 
           contains("Age 25 to 49") | 
           contains("Age 50 to 64") ) %>%
  #keeping only the data relating to Highest Level of Qualification: All, to give all residents 25 to 64 by ethnicity 
  select(contains("geography") | 
...
The optimal result for the MSOA data was produced using a an ensemble of two algorithms: Support Vector Machines with a radial kernel (svmRadial) and Generalized Additive Model using Splines (GAM), where the GAM model needed a reduction of numbers of features to 28 in order to complete the calculations.
...
The best models were linear regression, and variants of vector machines, gradient boosting, additive algorithms.  These last models, popular recently, seemed to work quite well, but computationally expensive.
...
In this case again an ensemble of 2 algorithms was best performing, Support Vector Machines with a radial kernel (svmRadial) again, but Stochastic Gradient Boosting (GBM) as the second.
...
The data set worked as a continuous machine learning problem, it behaved well, with sensible results and a variation in different results from different algorithms.  The census data prepared in this way also allowed for some interesting conclusions to be drawn from visualisations of the features.
...
|rmse |    0.043189|     0.013991|


### Model selection and construction

The machine learning analysis was able to make a significant stepwise improvement during the project by working with a lot of algorithms and constructing a final ensemble. The RMSE improved from a baseline "mean model" of 0.091347 to 0.015654 with the SVM Radial, and 0.014253 for the final ensemble for MSOA.  Similarly for the SOA, improving the RMSE from 0.1334185 to 0.0438095 with the SVM Radial, and 0.0431886 for the final ensemble.

This study focused on the data and features, and trying out a number of models, but without tuning them.  There is potential for detailed investigation and improvement for each model.
...
The project only briefly worked with the larger data sets from the SOA to verify that it worked, and there were problems with algorithms being able to handle both large numbers of features and observations, however it was possible to generate results.
...
In general interpretability from the models was not very clear.  Many have no indication of feature importance, or ways to get information out, without running many times with different sets of features.  Furthermore, many of the popular models for predictions using boosting or additive processes for iterative improvement which obfuscate the causality further.

In addition, the feature investigation at the end appeared to show some implications from the ethnicity features which were not highlighted in model outputs.  This may be because there were a lot of related features, so each on its own is less important (such as ethnicity and country of birth), in contrast to age, female ratio, lone parent which may not have so many related features, and hence are highlighted by the models.
...
By far and away the most important feature for a high quality occupation is qualifications.  As this is by geographic area, it is not clear on cause and effect: do highly qualified people all move to where the best jobs are, or are the best jobs where there are highly qualified people?  But the public policy implications are clear, ensuring all groups have access to education and qualifications is critical to ensure no one is left behind.  Or another way, if any cohort has poor qualification achievements they will have poor achievements in work, throughout their life.

Beyond that, the conclusions from the models was not clear, other than the country of birth did was not highlighted as an issue at all, ethnicity, households and so on were much more important, as was working in Industry KLMN (Financial and Real estate, Professional, scientific, and Administrative).

There were interesting implications from the analysis of the features, which align with recent political and press articles:

*  'white uk' was negatively correlated with both senior occupations and qualifications.  In contrast, other white ethnicities, "white Irish" and "white other" were both highly qualified and with the best jobs.  This could be the source of the anti-EU sentiment (Brexit), and populism in general within the white UK population, and supports the idea of "left behind" white working class communities discussed in the press.
*  poor occupation achievements for "black other" and "black African", despite good qualifications.  In contrast, white ethnicities generally have qualifications and occupation achievements aligned.  This is evidence of the UK variant of the Black Lives Matter movement, which is highlighting prejudice and lack of opportunities, "white privilege".  
* "lone parent" and "divorced" have clear negative impacts, implying poor roles for the residents in these households.  Possibly more support should be provided to these residents, probably around child care.

According to a recent study only 4.7% of the most powerful 1100 jobs are held by black, Asian and minority ethnic people, compared with the 13% proportion of the UK population. https://www.theguardian.com/business/2020/jul/28/bame-representation-uk-top-jobs-colour-of-power-survey 
...
There are many models which were not tried, another approach would be to move beyond the empirical model selection process and to use logic based on the specific data set to selecting specific models, and to find the best single model, to improve on the SVM Radial.
...
The analysis on numbers of features and observations could be improved, by taking a systematic approach, such as a measure of the computation expense against RMSE performance, for each step increase in observations or features, to identify the optimal point.  Similarly, computation expense can be used as part of model selection, RMSE improvement per CPU time.  This could be particularly helpful to select models using smaller data sets with the intention to be used on the larger data sets.


### Societal implications

The SOA data looked useful to draw conclusions from the features, but it would be better to use the full 130,000 observations for the whole of England and Wales rather than the subset in just two regions.

This study was focused on machine learning, but it could be extended and change focus to analyse the statistical implications of the data, the studying correlations, confounding data and statistical significance of the different factors/features.  And to extend the statistical analysis with other sources of data.
